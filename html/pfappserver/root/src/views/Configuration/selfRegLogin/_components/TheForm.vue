<template>
  ...
  >
    <form-group-sso-status namespace="sso_status"
                           ...
                           :text="$i18n.t('Whether or not SSO should be enabled for the self-registration login. Changing this requires to restart the httpd.portal service.')"
    ...
    <form-group-sso-login-path namespace="sso_login_path"
                               ...
                               :text="$i18n.t('The path to redirect the user to in order to perform the SSO login for self-registration.')"
    />

    <form-group-sso-login-text namespace="sso_login_text"
                               :column-label="$i18n.t('SSO Login Button Text')"
                               :text="$i18n.t('The text to display in the SSO login button for self-registration.')"
    />
    ...
    <form-group-allow-username-password namespace="allow_username_password"
                                        ...
                                        :text="$i18n.t('Whether or not username/password authentication is allowed for the self-registration login. Disabling this will force users to use SSO. Changing this requires to restart the httpd.portal service.')"
                                        enabled-value="enabled"
                                        disabled-value="disabled"
    />
